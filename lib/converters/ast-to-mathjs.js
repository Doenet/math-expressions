...
const operators = {
  "+": function (operands) {
    return operands.length === 1
      ? operands[0]
      : new math.OperatorNode("+", "add", operands);
  },
  "*": function (operands) {
    return new math.OperatorNode("*", "multiply", operands);
  },
  "/": function (operands) {
    return new math.OperatorNode("/", "divide", operands);
  },
  "-": function (operands) {
    return new math.OperatorNode("-", "unaryMinus", [operands[0]]);
  },
  "^": function (operands) {
    return new math.OperatorNode("^", "pow", operands);
  },
  ...
  vector: function (operands) {
    return new math.ArrayNode(operands);
  },
  ...
  and: function (operands) {
    return new math.OperatorNode("and", "and", operands);
  },
  or: function (operands) {
    return new math.OperatorNode("or", "or", operands);
  },
  not: function (operands) {
    return new math.OperatorNode("not", "not", [operands[0]]);
  },
  "<": function (operands) {
    return new math.OperatorNode("<", "smaller", operands);
  },
  ">": function (operands) {
    return new math.OperatorNode(">", "larger", operands);
  },
  le: function (operands) {
    return new math.OperatorNode("<=", "smallerEq", operands);
  },
  ge: function (operands) {
    return new math.OperatorNode(">=", "largerEq", operands);
  },
  ne: function (operands) {
    return new math.OperatorNode("!=", "unequal", operands);
  },
...
class astToMathjs {
  ...
  convert(tree) {
    if (typeof tree === "number") {
      ...
      if (tree < 0) return operators["-"]([new math.SymbolNode("Infinity")]);
    ...
    if (operator === "apply") {
      ...
      if (operands[0] === "factorial")
        return new math.OperatorNode("!", "factorial", [
          this.convert(operands[1]),
        ]);

      let functionWord = functionConversions[operands[0]];
      if (!functionWord) {
      ...
      if (operands[0] === "count") {
        if (f_args.length > 2 || !(f_args[0] instanceof math.ArrayNode)) {
          // Have a count function whose argument isn't a single array.
          // Turn in to a single argument of an array
    ...
    if (operator === "=") {
      ...
      for (let i = 1; i < arg_nodes.length; i++) {
        comparisons.push(
          new math.OperatorNode("==", "equal", arg_nodes.slice(i - 1, i + 1)),
        );
      }
    ...
    if (
      ...
      else comparisons.push(new math.OperatorNode(">", "larger", [x, a]));
      if (closed[2])
        comparisons.push(new math.OperatorNode("<=", "smallerEq", [x, b]));
      else comparisons.push(new math.OperatorNode("<", "smaller", [x, b]));
    ...
    if (
      ...
      if (small_closed[1] && !big_closed[1])
        comparisons.push(
          new math.OperatorNode(">", "larger", [small_a, big_a]),
        );
      else
        comparisons.push(
          new math.OperatorNode(">=", "largerEq", [small_a, big_a]),
        );

      if (small_closed[2] && !big_closed[2])
        comparisons.push(
          new math.OperatorNode("<", "smaller", [small_b, big_b]),
        );
      else
        comparisons.push(
          new math.OperatorNode("<=", "smallerEq", [small_b, big_b]),
        );
    ...
    if (operator === "array") {
      // Convert array into array node
      // Will become matrix on eval

      let result = operands.map((entry) => this.convert(entry));

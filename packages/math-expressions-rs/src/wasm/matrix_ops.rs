...
use crate::{to_text, Assumptions, Expr};
...
impl Expression {
    ...
    pub fn matrix_inverse(&self) -> Expression {
        Expression(crate::matrix_inverse(&self.0, &Assumptions::new()))
    }
    /// Reduced row-echelon form.
    pub fn rref(&self) -> Expression {
        Expression(crate::rref(&self.0, &Assumptions::new()))
    }
    /// Matrix rank, or `undefined` when not a literal matrix under the caps.
    pub fn rank(&self) -> Option<u32> {
        crate::rank(&self.0, &Assumptions::new())
    }
    ...
    pub fn nullspace(&self) -> Option<String> {
        let basis = crate::nullspace(&self.0, &Assumptions::new())?;
    ...
    pub fn eigenvalues(&self) -> Option<String> {
        let vals = crate::matrix::eigenvalues(&self.0, &Assumptions::new())?;
    ...
    pub fn eigenvectors(&self) -> Option<String> {
        let pairs = crate::matrix::eigenvectors(&self.0, &Assumptions::new())?;

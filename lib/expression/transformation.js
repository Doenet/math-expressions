...
function expand(expr_or_tree, no_division) {
  ...
  // Distributing a `pm`-containing factor across a sum is unsound:
  // `(±k)·(b+c)` has 2 values, but `(±k)·b + (±k)·c` has 4 (the two
  // copies of `±k` become independent sign choices). Distributing
  // `pm`-containing operands of a power likewise inflates the value
  // set, e.g. `(±k+m)^3 → ±k^3 + 3k^2 m + ±(3k m^2) + m^3` has
  // sign-combinations that the original cannot produce. Even squaring
  // is unsafe for a *composite* pm operand: `(m+(n±x))^2` duplicates
  // the `(n±x)` subtree into the squared term and the cross term,
  // making those sign choices independent. So guard the outer factor /
  // power base against `pm` for all the expansion rules below.
  const no_dx_no_pm = (v) => no_dx(v) && !contains_pm(v);
  const no_pm = (v) => !contains_pm(v);
  ...
  if (!no_division)
    transformations.push([
      textToAst.convert("(a+b)/c"),
      textToAst.convert("a/c+b/c"),
      { variables: { a: true, b: true, c: no_pm } },
    ]);
  transformations.push([
  ...
  transformations.push([
    ...
    { variables: { a: no_pm, b: true, c: true, d: true } },
  ]);

...
function matchOperands(operator, treeOperands, patternOperands, params,
  ...
  if(params.allow_permutations) {
    if (operator === "*") {
      allow_permutations = ![...treeOperands, ...patternOperands].some(x =>
        ["tuple", "list", "vector", "altvector", "interval", "set", "array", "matrix"].includes(x[0])
      )
  ...
  function matchOps(treeOpIndicesLeft, patternInd, matches) {
    ...
    if(flatten.is_associative[operator] || ["tuple", "vector", "altvector"].includes(operator)) {
...
function matchImplicitIdentity(tree, pattern, params) {
  ...
  if(implicit_identities.length === 0)
  	return false;

  for(let implicit_identity of implicit_identities) {
    let matches = match_one_implicit_identity(tree, pattern, params, implicit_identity)
    if(matches) {
      return matches;
    }
  }

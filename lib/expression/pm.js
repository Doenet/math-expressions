...
const MAX_PM_EXPANSIONS = 1024; // 2^10 — cap on sign enumerations
...
// (sign = -). Throws if the count would exceed MAX_PM_EXPANSIONS.
export function expand_pm_signs(tree) {
  const n = count_pm(tree);
  if (n === 0) return [tree];
  if (n > 10) {
    throw new Error(
      `pm: cannot expand ${n} plus-minus operators (limit is 10 → ${MAX_PM_EXPANSIONS} combinations)`,
    );
  }
  const total = 1 << n;
  const results = [];

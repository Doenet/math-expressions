...
const component_equals = function ({ expr, other, randomBindings,
  ...
  // (remove any exponents and include if part before subscript is single character)
  var functions = [...new Set([
    ...expr.functions().map(x => x[0] === "^" ? x[1] : x),
    ...other.functions().map(x => x[0] === "^" ? x[1] : x)
  ])].filter(x => x.length === 1 || x[1] === "_");
  ...
  if (function_variable_overlaps.length > 0) {
    ...
    for (let fvar of function_variable_overlaps) {
      ...
      let tranformations = [
        ...
        [
          ['apply', ['^', fvar, varForExp], varForArg],
          ['apply', ['^', newFun, varForExp], varForArg],
          { variables: { [varForArg]: true , [varForExp]: true } }
        ],
      ]

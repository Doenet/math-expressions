...
import { contains_pm, expand_pm_signs } from "../pm.js";
import { pm_equals_numerical } from "./pm-numerical.js";

var astToText = new astToTextObj();

export function generate_random_integer(minvalue, maxvalue, rng) {
  minvalue = math.ceil(minvalue);
  maxvalue = math.floor(maxvalue);
  return math.floor(rng() * (maxvalue - minvalue + 1)) + minvalue;
}

// Build a random univariate cubic with coefficients drawn from `rng`,
// used as a stand-in for unknown function variables when sampling values.
export function random_cubic(rng) {
  const c1 = (rng() - 0.5) * 1;
  const c2 = (rng() - 0.5) * 2;
  const c3 = (rng() - 0.5) * 6;
  const c4 = (rng() - 0.5) * 24;
...
export const equals = function ({
  ...
  if (!expr_context) expr_context = expr.context;
  if (!other_context) other_context = other.context;
  ...
  if (Array.isArray(expr.tree) && Array.isArray(other.tree)) {
    ...
    if (
      ...
      for (let i = 0; i < expr_operands.length; i++) {
        // Recurse on each component; the recursive call will re-check for
        // pm at its top level, so pm inside a tuple/vector component is
        // handled correctly by the pm dispatch below.
        if (
          !equals({
    ...
    if (
      ...
      // If either side of the standard-form relation contains pm, dispatch
      // to the pm-aware comparison.
      if (contains_pm(expr_rhs) || contains_pm(other_rhs)) {
        if (expr_operator === "=") {
          // An equation `lhs = rhs` with pm denotes a disjunction of equations
          // (one per sign choice); its solution set is the zero set of the
          // PRODUCT of the sign-expansion branches of the standard form. Two
          // such equations are equal iff those products are proportional, so
          // reuse `component_equals`'s allow_proportional logic on the
          // products. This recovers the scaling invariance the non-pm `=` path
          // has — e.g. `x = 5 ± 3` and `2x = 10 ± 6` share the solution set
          // {2, 8} and must compare equal.
          // Deduplicate equivalent sign-expansion branches so each distinct
          // factor appears once. Without this, a vacuous pm operand (e.g.
          // `± 0`) or two coincident sign choices yield duplicate branches,
          // inflating the product's multiplicity. For example `x = 5 ± 0`
          // expands to `[x-5, x-5]` with product `(x-5)^2`, which is not
          // proportional to `x = 5`'s product `(x-5)` even though they are the
          // same equation. Comparing the zero-set without preserving branch
          // multiplicity collapses these to a single factor.
          const dedupe_product = (rhs, context) => {
            const branches = expand_pm_signs(rhs);
            const kept = [];
            for (const branch of branches) {
              const simplified = context.fromAst(branch).simplify().tree;
              if (!kept.some((k) => tree_equal(k, simplified))) {
                kept.push(simplified);
              }
            }
            // A single remaining factor is the product itself; wrapping it in
            // `["*", ...]` would be a malformed unary product.
            return kept.length === 1 ? kept[0] : ["*", ...kept];
          };
          let expr_product, other_product;
          try {
            expr_product = dedupe_product(expr_rhs, expr_context);
            other_product = dedupe_product(other_rhs, other_context);
          } catch (e) {
            // too many pm operators to enumerate
            return false;
          }
          return component_equals({
            expr: expr_context.fromAst(expr_product),
            other: other_context.fromAst(other_product),
            randomBindings: randomBindings,
            expr_context: expr_context,
            other_context: other_context,
        ...
        // Inequalities: proportionality is ambiguous for set-valued relations,
        // so compare the standard-form value multisets directly.
        return pm_equals_numerical({
  ...
  // pm dispatch for non-tuple, non-relation cases. Tuples/vectors recurse
  // above (each component re-enters `equals` and hits this dispatch if it
  // contains pm); relations dispatch above after standard-form conversion.
  if (contains_pm(expr.tree) || contains_pm(other.tree)) {
    return pm_equals_numerical({
      expr,
      other,
      randomBindings,
...
const component_equals = function ({
  ...
  function find_equality_region(noninteger_scale, rng) {
    ...
    for (let i = 0; i < functions.length; i++) {
      bindings[functions[i]] = random_cubic(rng);
    }
    ...
    for (let j = 0; j < 100; j++) {
      ...
      // replace any function variables with a function
      for (let i = 0; i < functions.length; i++) {
        bindings2[functions[i]] = random_cubic(rng);
      }

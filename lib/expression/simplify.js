...
function collect_like_terms_factors(expr_or_tree, assumptions, max_digits) {
  ...
  // Predicate used by like-terms rules below. Combining `n*x + m*x → (n+m)*x`
  // is unsound when `x` carries an independent ± sign — e.g. `±3 + ±3` has
  // value set {−6, 0, 6} but `2·±3` has only {−6, 6}.
  const isLikeTermBase = (v) => !contains_pm(v);
  // n*x + m*x -> (n + m)*x
  transformations.push([
    ["+", ["*", "n", "x"], ["*", "m", "x"]],
    ["*", ["+", "n", "m"], "x"],

...
export const equals = function ({
  ...
  if (Array.isArray(expr.tree) && Array.isArray(other.tree)) {
    ...
    if (
      ...
      // to the pm-aware comparison. Note: proportionality (used for
      // inequalities) is ambiguous for set-valued expressions, so pm-bearing
      // relations are compared as multisets of values directly (i.e. the
      // `allow_proportional` / `require_positive_proportion` flags are
      // effectively dropped for pm-bearing relations).
...
// Numeric equality for expressions containing `pm`.
//
...
// (within tolerance). This function expands all `pm` operators into their
// 2^n sign assignments, then samples bindings and compares the resulting
// value collections via `value_multisets_match`.
function pm_equals_numerical({
  expr,
  other,
  randomBindings,
  expr_context,
  ...
  // Build a tolerance function from the numbers appearing in the expression
  // (mirroring `component_equals`). pm-expansion does not change which
  // numeric literals appear in the expression, so we use the first variant
  // (all `pm` chosen as `+`) so that `derivative` doesn't have to traverse
  // pm nodes; the resulting magnitude is a representative tolerance.
  let parameters_for_numbers;
  let tolerance_function;
  if (allowed_error_in_numbers > 0 && expr_variant_trees.length > 0) {
    try {
      let result = replace_numbers_with_parameters({
        expr: expr_context.fromAst(expr_variant_trees[0]),
        variables: variables,
        include_exponents: include_error_in_number_exponents,
      });
      let expr_with_params = expr_context.fromAst(result.expr_with_params);
      parameters_for_numbers = result.parameters;
      ...
      if (parameter_list.length > 0) {
        ...
        let tolerance_expression =
          derivative_sum.multiply(allowed_error_in_numbers);

        try {
          tolerance_function = tolerance_expression.f();
        } catch (e) {
          // can't create function out of derivative; leave undefined
        }
      }
    } catch (e) {
      // unable to build tolerance function; fall back to plain tolerances
    }
  }
  ...
  for (let i = 0; i < 10 * number_tries; i++) {
    ...
    if (tolerance_function) {
      ...
      try {
        number_tolerance = math.abs(
          tolerance_function(bindingsIncludingParameters),
        );
      } catch (e) {
        continue;
      }
      if (!Number.isFinite(number_tolerance)) {
        continue;
      }
    }

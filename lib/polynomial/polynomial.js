...
function mono_div(top, bottom) {
    ...
    if ( !Array.isArray(bottom) || bottom[0] !== "monomial"){
        ...
        if ( !Array.isArray(top) || top[0] !== "monomial")      //if top is constant
            ...
            return [top[0], ['/', top[1], bottom], top[2]];       //shouldn't be passing constants other than 1
    }
    ...
    if (bottom[1] == 1)
        ...
        return ["monomial", ['/', top[1], bottom[1]], div_vars];
}
...
function hij(i, j, polys){
    ...
    let std_exp = poly_div(polynomial_sub(polynomial_mul(mji, polys[i]), polynomial_mul(mij, polys[j])), polys);        //HERE
    return std_exp[1];
}
...
function reduce_rational_expression(top, bottom){
    ...
    let new_top = poly_by_divisor(top, gcd);
    let new_bottom = poly_by_divisor(bottom, gcd);

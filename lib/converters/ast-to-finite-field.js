...
function finite_field_evaluate_ast(tree, bindings, modulus, level) {
  ...
  if (operator === "apply") {
    ...
    if (operands[0] === "sec") {
      return finite_field_evaluate_ast(['/', 1, ['apply', 'cos', operands[1]]], bindings, modulus, level);
    }

    if (operands[0] === "csc") {
      return finite_field_evaluate_ast(['/', 1, ['apply', 'sin', operands[1]]], bindings, modulus, level);
    }

    if (operands[0] === "cot") {
      return finite_field_evaluate_ast(['/', 1, ['apply', 'tan', operands[1]]], bindings, modulus, level);
    }

    if (operands[0] === "tan") {
      var phi = numberTheory.eulerPhi(modulus);
      return finite_field_evaluate_ast(['/', ['apply', 'sin', operands[1]], ['apply', 'cos', operands[1]]], bindings, modulus, level);
    }
    ...
    if(operands[0] === "sqrt") {
      return finite_field_evaluate_ast(operands[1], bindings, modulus, level).sqrt();
    }

    if(operands[0] === "abs") {
      let operand1 = finite_field_evaluate_ast(operands[1], bindings, modulus, level);

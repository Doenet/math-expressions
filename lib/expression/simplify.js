...
function simplify(expr_or_tree, assumptions) {
    var tree = get_tree(expr_or_tree);
...
function evaluate_numbers_sub(tree, assumptions) {
	...
	if(typeof numer === 'number') {
		...
		if(math.round(quotient,4) == quotient)
		    return quotient;
		else if (denom < 0)
		    return ['/', -numer, -denom];
	    }
...
function evaluate_numbers(expr_or_tree, assumptions) {
    ...
    var result = default_order(evaluate_numbers_sub(tree, assumptions));

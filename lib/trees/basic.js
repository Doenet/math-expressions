...
import { default_order } from "./default_order.js";
import * as flatten from "./flatten.js";
import * as simplify from "../expression/simplify.js";
import { subsets } from "./util.js";
import { coerce_tuple_array_vectors } from "../expression/equality/coersion.js";
...
export const equal = function (
  ...
  if (leftOperator !== rightOperator) {
    ({
      operator1: leftOperator,
      operator2: rightOperator,
      operands1: leftOperands,
      operands2: rightOperands,
  ...
  if (pattern in variables) {
    ...
    if (condition !== true) {
      if (condition instanceof RegExp) {
        ...
        if (!variables[pattern](tree)) return false;
      } else {
        return false;
      }
    }
...
function matchOperands(
  ...
  if (params.allow_permutations) {
    if (operator === "*") {
      allow_permutations = ![...treeOperands, ...patternOperands].some((x) =>
        [
          "tuple",
          "list",
          "vector",
  ...
  function matchOps(treeOpIndicesLeft, patternInd, matches) {
    ...
    if (
      flatten.is_associative[operator] ||
      ["tuple", "vector", "altvector"].includes(operator)
    ) {
    ...
    for (let inds of inds_set) {
      ...
      if (
        !_.every(
          _.intersection(Object.keys(matches), Object.keys(m)),
          function (k) {
...
function matchImplicitIdentity(tree, pattern, params) {
  ...
  for (let i = 0; i < patternOperands.length; i++) {
    ...
    if (
      ...
      implicit_identities.push(po);
    }
  }

  if (implicit_identities.length === 0) return false;

  for (let implicit_identity of implicit_identities) {
    let matches = match_one_implicit_identity(
      tree,
      pattern,
      params,
      implicit_identity,
...
function match_one_implicit_identity(tree, pattern, params, implicit_identity) {
  var operator = pattern[0];
  var patternOperands = pattern.slice(1);
      ...
      new_tree = transform(new_tree, function (subtree) {
        ...
        if (m) {
          ...
          const assumptions = params.assumptions
            ? substitute(params.assumptions, m)
            : undefined;
...
export const applyTransformationEachSubtree = function (
  tree,
  pattern,
  replacement,
) {
...
export const equalAfterTransformations = function (
  left,
  right,
  transformers,
  depth,
  ...
  if (comparer === undefined) comparer = equal;
    ...
    for (let item of queue) {
      for (let transformer of transformers) {
        ...
        for (let result of results) {
          if (
            _.every(queue, function (other) {
              return !comparer(result, other);
            })
          )

...
function value_multisets_match(
  ...
  // Edge predicate: can LHS element i be paired with RHS element j? The
  // tolerance is keyed on i because only the LHS carries numeric-error
  // tolerance.
  const can_match = (i, j) =>
    values_close(
      values_a[i],
  ...
  if (values_a.length !== values_b.length) {
    ...
    return (
      values_a.every((_, i) => values_b.some((_, j) => can_match(i, j))) &&
      values_b.every((_, j) => values_a.some((_, i) => can_match(i, j)))
    );
  }

  // Equal cardinality: require a perfect one-to-one pairing. Use augmenting
  // paths (Kuhn's algorithm for maximum bipartite matching) rather than
  // greedy first-fit. With per-variant tolerances the "close" relation is not
  // symmetric across variants, so a greedy choice can let an earlier, looser
  // LHS variant consume the only RHS value a later, tighter variant could
  // match — spuriously failing even though a complete pairing exists (e.g.
  // `95 ± 5` vs `95 ± (-4)` with allowed_error_in_numbers 0.1, where the only
  // valid pairing is 100↔99 and 90↔91). Augmenting paths are order-independent.
  const n = values_a.length;
  const matchB = new Array(n).fill(-1); // matchB[j] = LHS index paired with RHS j

  const tryMatch = (i, visited) => {
    for (let j = 0; j < n; j++) {
      if (visited[j] || !can_match(i, j)) continue;
      visited[j] = true;
      if (matchB[j] === -1 || tryMatch(matchB[j], visited)) {
        matchB[j] = i;
        return true;
      }
    }
  ...
  for (let i = 0; i < n; i++) {
    if (!tryMatch(i, new Array(n).fill(false))) {
      return false;
    }
  }
  return true;

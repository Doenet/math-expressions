...
const glslOperators = {
  ...
  sinh: function (operands) {
    return "csinh(" + operands[0] + ")";
  },
  cosh: function (operands) {
    return "ccosh(" + operands[0] + ")";
  },
...
class astToGLSL {
  ...
  convert(tree) {
    ...
    if (typeof tree === "string") {
      if (tree === "e") return "vec2(2.71828182845905,0.0)";

      if (tree === "pi") return "vec2(3.14159265358979,0.0)";

      if (tree === "i") return "vec2(0.0,1.0)";
    ...
    if ("real" in tree && "imaginary" in tree) return tree;
    ...
    if (operator === "apply") {
      if (typeof operands[0] !== "string")
        throw Error(
          "Non string functions not implemented for conversion to GLSL",
        );

      var operator = operands[0];
      var operands = operands.slice(1);

      return glslOperators[operator](
        operands.map(
          function (v, i) {

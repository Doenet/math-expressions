...
class ZmodN {
  constructor(values, modulus, approximate = false) {
    this.values = values.map(function (x) {
      return positiveModulo(x, modulus);
    });
  ...
  apply(other, callback, modulus) {
    if (modulus == undefined || Number.isNaN(modulus)) {
      if (Number.isNaN(this.modulus) || Number.isNaN(other.modulus))
        return new ZmodN([NaN], NaN, this.approximate || other.approximate);
        ...
        this.values.map(function (x) {
          return other.values.map(function (y) {
            if (Number.isNaN(x) || Number.isNaN(y)) {
              return NaN;
            } else return (modulus + callback(x, y)) % modulus;
          });
        }),
      ),
      modulus,
  ...
  divide(other) {
      ...
      flatten(
        this.values.map(function (b) {
          return other.values.map(function (a) {
            ...
            var x0 = numberTheory.multiplyMod(
              bd,
              numberTheory.inverseMod(ad, m),
              m,
            );
    ...
    return new ZmodN(values, m, this.approximate || other.approximate);
  }

...
var real_math_functions = {
  ...
  arccsc: function (operands) {
    return Math.asin(1.0 / operands[0]);
  },
  arcsec: function (operands) {
    return Math.acos(1.0 / operands[0]);
  },
  arccot: function (operands) {
    return Math.atan(1.0 / operands[0]);
  },

  csc: function (operands) {
    return 1.0 / Math.sin(operands[0]);
  },
  sec: function (operands) {
    return 1.0 / Math.cos(operands[0]);
  },
  ...
  factorial: function (operands) {
    return new ComplexNumber(operands[0], 0.0).factorial().real;
  },
  gamma: function (operands) {
    return new ComplexNumber(operands[0], 0.0).gamma().real;
  },
...
function real_evaluate_ast(tree, bindings) {
  if (typeof tree === "string") {
    if (tree === "e") return Math.E;

    if (tree === "pi") return Math.PI;

    if (tree in bindings) return bindings[tree];
  ...
  if (operator in real_math_functions) {
    return real_math_functions[operator](

...
import { perform_matrix_scalar_multiplications, perform_vector_scalar_multiplications } from './transformation';
import { get_all_units, get_unit_of_tree, get_unit_value_of_tree } from './units';
...
function perform_vector_matrix_additions_scalar_multiplications(expr_or_tree, include_tuples = true) {
  var tree = get_tree(expr_or_tree);
    ...
    if (matchResults) {
      ...
      for (let n in vectorAddendsByLength) {
        if (vectorAddendsByLength[n].length < 2) {
          ...
          let foundTuple = vectorAddendsByLength[n].some(x => x[0] === "tuple");
          let foundAltVector = vectorAddendsByLength[n].some(x => x[0] === "altvector");
...
function contains_only_numbers(tree, { include_number_symbols = false, evaluate_functions = false } = {}) {
  if (typeof tree === "string") {
  ...
  if (evaluate_functions && tree[0] === "apply") {
    return tree.slice(2).every(x => contains_only_numbers(x, { include_number_symbols: include_number_symbols }));
  }
...
function try_evaluate_negate_number(tree) {
  ...
  if (Array.isArray(tree) && tree[0] === '/') {
    ...
    if (Array.isArray(tree[1]) && tree[1][0] === '*'
      && (typeof tree[1][1] === 'number')) {
      return ['/', [
        '*', -tree[1][1]].concat(tree[1].slice(2)),
        tree[2]];
...
function try_evaluate_quotient_of_numbers(numer, denom, assumptions, max_digits) {
...
function evaluate_numbers_sub(tree, assumptions, max_digits, skip_ordering, evaluate_functions, set_small_zero) {
  ...
  if (contains_only_numbers(tree, { include_number_symbols: true, evaluate_functions })) {
    ...
    let simplify_number = function (c) {
      ...
      // return the number
      if (contains_decimal_number(tree) && contains_only_numbers(tree, { include_number_symbols: true, evaluate_functions })) {
        return c;
      }
  ...
  if (operator === '+') {
    ...
    if (left[0] === "unit" && right[0] === "unit") {
      ...
      if (left_unit && left_unit === right_unit) {
        ...
        let result = try_evaluate_sum_of_numbers(left_without_unit, right_without_unit, skip_ordering);
  ...
  if (operator === '/') {
    ...
    let result = try_evaluate_quotient_of_numbers(numer, denom, assumptions, max_digits);
    ...
    if (numer[0] === "unit" && denom[0] !== "unit") {
      ...
      if (unit) {
        ...
        let result = try_evaluate_quotient_of_numbers(numer_without_unit, denom, assumptions, max_digits);
...
function remove_units(expr_or_tree, scale_based_on_unit = true) {
  ...
  if (operator === "unit") {
    ...
    let value = remove_units(unit_object.prefix ? operands[1] : operands[0], scale_based_on_unit);
    ...
    return [operator, ...operands.map(x => remove_units(x, scale_based_on_unit))];
  }
}
...
function remove_scaling_units(expr_or_tree) {
  ...
  if (operator === "unit") {
    let unit_object = all_units[get_unit_of_tree(tree)];
    if (!unit_object?.only_scales) {
      return tree;
    }
    let value = remove_scaling_units(unit_object.prefix ? operands[1] : operands[0]);
    return unit_object.scale(value)
  } else {
...
export {
  clean, simplify, simplify_logical, evaluate_numbers, collect_like_terms_factors,
  collapse_unary_minus, simplify_ratios, default_order,
  perform_vector_matrix_additions_scalar_multiplications,
  remove_units, add_unit, remove_scaling_units
};

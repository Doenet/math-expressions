...
var parser_defaults = require('./converters/parser-defaults');
var assumptions = require('./assumptions/assumptions');
var math=require('./mathjs');
var flatten = require('./trees/flatten');
var expression_to_tree = require('./expression' ).expression_to_tree;
var expression_to_other = require('./expression' ).expression_to_other;
var functions_to_tree = require('./functions' ).expression_to_tree;
...
function create_from_multiple(expr, pars) {
	...
	try {
	    return new Expression( parser.text.to.ast(expr, context, pars), context );
	}
	catch(e_text) {
	    try {
...
function parseText(string, pars) {
    var context = this;
    return new Expression( parser.text.to.ast(string, context, pars), context);
};

...
import { substitute } from "../trees/basic.js";
...
var derivatives = {
  ...
  cbrt: textToAst.convert("1/(3*x^(2/3))"),
  ...
  asin: textToAst.convert("1/sqrt(1 - x^2)"),
  acos: textToAst.convert("-1/sqrt(1 - x^2)"),
  atan: textToAst.convert("1/(1 + x^2)"),
  acsc: textToAst.convert("-1/(sqrt(-1/x^2 + 1)*x^2)"),
  asec: textToAst.convert("1/(sqrt(-1/x^2 + 1)*x^2)"),
  acot: textToAst.convert("-1/(1 + x^2)"),
  abs: textToAst.convert("abs(x)/x"),
};

function derivative_with_story(expr_or_tree, x, story = []) {
  ...
  if (typeof tree === "string") {
    if (x === tree) {
      story.push(
        "We know the derivative of the identity function is one, that is, \\(" +
          ddx +
          astToLatex.convert(tree) +
          " = 1\\).",
      );
  ...
  if (operator === "apply" && operands[0] === "nthroot") {
    if (operands[1].length !== 3 || operands[1][0] !== "tuple") {
      // treat as square root if not in form of nthroot(x, n)
      operands[0] = "sqrt";
    } else {
      // derivative of nthroot is the same as derivative of x^(1/n)
      let n = operands[1][2];
      operator = "^";
      operands = [operands[1][1], ["/", 1, n]];
    }
  }
  ...
  if (operator === "+" || operator === "-" || operator === "~") {
    story.push(
      "Using the sum rule, \\(" +
        ...
        operands
          .map(function (v, i) {
            return ddx + astToLatex.convert(v);
          })
          .join(" + ") +
        "\\).",
    );
    let result = [operator].concat(
      operands.map(function (v, i) {
        return derivative_with_story(v, x, story);
      }),
    );
  ...
  if (operator === "*") {
    ...
    for (let i = 0; i < operands.length; i++) {
      if (
        typeof operands[i] === "number" ||
        variables(operands[i]).indexOf(x) < 0
      ) {
    ...
    if (numeric_operands.length > 0) {
      ...
      story.push(
        "By the constant multiple rule, \\(" +
          ...
          ddx +
          "\\left(" +
          astToLatex.convert(remaining) +
          "\\right)\\).",
      );

      let d = derivative_with_story(remaining, x, story);
          ...
          .map(function (v, i) {
            return operands
              .map(function (w, j) {
                ...
                else return astToLatex.convert(w);
              })
              .join(" \\cdot ");
          })
          .join(" + ") +
      ...
      operands.map(function (v, i) {
        return ["*"].concat(
          inner_operands
            .map(function (w, j) {
              if (i === j) {
                let d = derivative_with_story(w, x, story);
  ...
  if (operator === "/") {
    ...
    if (variables(g).indexOf(x) < 0) {
      story.push(
        "By the constant multiple rule, \\(" +
          ...
          astToLatex.convert(tree) +
          " = " +
          astToLatex.convert(["/", 1, g]) +
          " \\cdot " +
          ddx +
          "\\left(" +
          astToLatex.convert(f) +
          "\\right)\\).",
      );

      let df = derivative_with_story(f, x, story);
    ...
    if (variables(f).indexOf(x) < 0) {
      ...
      story.push(
        "Since \\(\\frac{d}{du} \\frac{1}{u}\\) is \\(\\frac{-1}{u^2}\\), the chain rule gives \\(" +
          ddx +
          astToLatex.convert(tree) +
          " = " +
          astToLatex.convert(f) +
          "\\cdot \\frac{-1}{ " +
          astToLatex.convert(g) +
          "^2} \\cdot " +
          ddx +
          astToLatex.convert(g) +
          "\\).",
      ...
      let a = derivative_with_story(g, x, story);

      let quotient_rule = textToAst.convert("f * (-a/(g^2))");
      let result = substitute(quotient_rule, { f: f, a: a, g: g });
      result = simplify(result);
      story.push(
        "So since \\(\\frac{d}{du} \\frac{1}{u}\\) is \\(\\frac{-1}{u^2}\\), the chain rule gives \\(" +
          ddx +
          astToLatex.convert(tree) +
          " = " +
          astToLatex.convert(result) +
    ...
    let a = derivative_with_story(f, x, story);
    let b = derivative_with_story(g, x, story);
  ...
  if (operator === "^") {
    ...
    if (variables(exponent).indexOf(x) < 0) {
      if (typeof base === "string" && base === "x") {
        if (typeof exponent === "number") {
          ...
          let result = substitute(power_rule, {
            n: exponent,
            m: exponent - 1,
            f: base,
          });
          ...
          story.push(
            "By the power rule, \\(" +
              ...
              astToLatex.convert(tree) +
              " = " +
              astToLatex.convert(exponent) +
              " \\cdot \\left(" +
              astToLatex.convert(base) +
              "\\right)^{" +
              astToLatex.convert(["-", exponent, 1]) +
              "}\\).",
          );
          return result;
        }
      ...
      let a = derivative_with_story(base, x, story);
      ...
      if (typeof exponent === "number") {
        ...
        result = simplify(result);
        story.push(
          "So by the power rule and the chain rule, \\(" +
            ddx +
            astToLatex.convert(tree) +
            " = " +
            astToLatex.convert(result) +
    ...
    if (base === "e" && math.define_e) {
      ...
      story.push(
        "Using the rule for \\(e^x\\) and the chain rule, we know \\(" +
          ddx +
          astToLatex.convert(tree) +
          " = " +
          astToLatex.convert(tree) +
      ...
      let d = derivative_with_story(exponent, x, story);
      let result = substitute(power_rule, { f: exponent, d: d });
      result = simplify(result);
      story.push(
        "So using the rule for \\(e^x\\) and the chain rule, \\(" +
          ddx +
          astToLatex.convert(tree) +
          " = " +
          astToLatex.convert(result) +
    ...
    if (typeof base === "number") {
      ...
      story.push(
        "Using the rule for \\(a^x\\) and the chain rule, we know \\(" +
          ddx +
          astToLatex.convert(tree) +
          " = " +
          astToLatex.convert(partial_result) +
          " \\cdot " +
          ddx +
          astToLatex.convert(exponent) +
          "\\).",
      ...
      result = simplify(result);
      story.push(
        "So using the rule for \\(a^x\\) and the chain rule, \\(" +
          ddx +
          astToLatex.convert(tree) +
          " = " +
          astToLatex.convert(result) +
    ...
    story.push(
      "Recall the general rule for exponents, namely that \\(\\frac{d}{dx} u(x)^{v(x)} = u(x)^{v(x)} \\cdot \\left( v'(x) \\cdot \\log u(x) + \\frac{v(x) \\cdot u'(x)}{u(x)} \\right)\\).  In this case, \\(u(x) = " +
        astToLatex.convert(f) +
        "\\) and \\(v(x) = " +
        astToLatex.convert(g) +
        "\\).",
    ...
    result = simplify(result);
    story.push(
      "So by the general rule for exponents, \\(" +
        ddx +
        astToLatex.convert(tree) +
        " = " +
        astToLatex.convert(result) +
  ...
  if (operator === "apply" && !(operands[0] in derivatives)) {
    ...
    let result = [
      "*",
      substitute(["apply", operands[0] + "'", "x"], { x: input }),
      derivative_with_story(input, x, story),
    ];
    result = simplify(result);
    story.push(
  ...
  // chain rule
  if (
    (operator === "apply" && operands[0] in derivatives) ||
    ...
    if (typeof input === "number") {
      ...
      let result = ["*", substitute(derivatives[operator], { x: input })];
      result = simplify(result);
      story.push(
        "It is the case that \\(" +
          ddx +
      ...
      if (used_apply) example_ast = ["apply"].concat(example_ast);
      story.push(
        "Recall \\(\\frac{d}{du}" +
          astToLatex.convert(example_ast) +
          " = " +
          astToLatex.convert(derivative_with_story(example_ast, "u", [])) +
          "\\).",
      );
      ...
      story.push(
        "By the chain rule, \\(" +
          ...
          astToLatex.convert(tree) +
          " = " +
          astToLatex.convert(substitute(derivatives[operator], { x: input })) +
          " \\cdot " +
          ddx +
          astToLatex.convert(input) +
          "\\).",
...
function simplify_story(story) {
  ...
  for (let i = story.length - 1; i >= 1; i--) {
    if (story[i] === story[i - 1]) story.splice(i, 1);
  }
...
function derivative_story(expr, x) {
  var story = [];
  derivative_with_story(expr, x, story);
...
function derivative(expr_or_tree, x) {
  var tree = get_tree(expr_or_tree);

  const mt = astToMathjs.convert(
  ...
  const df = math.derivative(mt, x);

  return mathjsToAst.convert(df);
}

export { derivative, derivative_with_story, derivative_story, derivativeStory };

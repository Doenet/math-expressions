...
class astToLatex {
  ...
  simple_factor_or_function_or_parens(tree) {
    ...
    // or tree is a string
    // or tree is a function call other than sqrt, cbrt, or nthroot
    ...
    if (
      ...
      (Array.isArray(tree) &&
        tree[0] === "apply" &&
        !["sqrt", "cbrt", "nthroot"].includes(tree[1])) ||
  ...
  factor(tree) {
    ...
    if (operator === "^") {
      ...
      if (operands[0] === "abs") {
        ...
        return "\\sqrt[3]{" + this.statement(operands[1]) + "}";
        ...
        if (
          ...
          return (
            "\\sqrt[" +
            this.statement(operands[1][2]) +
            "]{" +
            this.statement(operands[1][1]) +
            "}"
          );
        } else {
          return "\\sqrt{" + this.statement(operands[1]) + "}";
  ...
  termWithPlusIfNotNegated(tree) {
    ...
    if (result.match(/^\\pm /)) return result;

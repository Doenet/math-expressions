...
describe("normalize function names", function () {
  ...
  _.each(_.keys(trees), function (string) {
    it(string, function () {
      expect(me.from(string).normalize_function_names().tree).toEqual(
        trees[string],
      );
    });
  });
...
describe("normalize applied functions", function () {
  ...
  it("neg 1 exponent not normalized outside for trig b", function () {
    expect(
      me.normalize_applied_functions(me.from("\\sin^(-1)(x)")).tree,
  ...
  it("derivative exponent outside", function () {
    expect(me.from("f(x)'^2").tree).toEqual([
      "^",
      ["prime", ["apply", "f", "x"]],
      2,
    ]);
...
describe("normalize tuples", function () {
  ...
  it("interval and vector3", function () {
    expect(
      me.from("(x,y)+(x,y,z)").to_intervals().tuples_to_vectors().tree,
    ).toEqual([
      "+",
...
describe("convert subscripts to strings", function () {
  ...
  it("subscripts embedded", function () {
    expect(me.from("x_2^y").subscripts_to_strings().tree).toEqual([
      "^",
      "x_2",
      "y",
    ]);
  ...
  it("complex subscripts skipped", function () {
    expect(me.from("(x^3)_2").subscripts_to_strings(true).tree).toEqual(
      "(x^3)_2",
    );
  });
});
...
describe("convert strings to subscripts", function () {
  it("simple subscripts", function () {
    expect(me.fromAst("x_2").strings_to_subscripts().tree).toEqual([
      "_",
      "x",
      2,
    ]);
    expect(me.fromAst("x_y").strings_to_subscripts().tree).toEqual([
      "_",
      "x",
      "y",
    ]);
    expect(me.fromAst("2_2").strings_to_subscripts().tree).toEqual(["_", 2, 2]);
    expect(me.fromAst("3_y").strings_to_subscripts().tree).toEqual([
      "_",
      3,
      "y",
    ]);
  ...
  it("subscripts embedded", function () {
    expect(me.fromAst(["^", "x_2", "y"]).strings_to_subscripts().tree).toEqual([
      "^",
      ["_", "x", 2],
      "y",
    ]);
    expect(
      me.fromAst(["^", ["apply", "sin", "x_t"], "y_3"]).strings_to_subscripts()
        .tree,
    ).toEqual(["^", ["apply", "sin", ["_", "x", "t"]], ["_", "y", 3]]);
  });
});
...
describe("default order", function () {
  ...
  it("order set operations", function () {
    expect(
      me.from("(A union B) intersect (C union D)").default_order().tree,
    ).toEqual(
  ...
  it("order containment", function () {
    expect(
      me.from("a elementof A and b notelementof B").default_order().tree,
    ).toEqual(
      me.from("B notcontainselement b and A containselement a").default_order()
        .tree,
    );

    expect(
      me.from("A superset B or C notsubset D").default_order().tree,
    ).toEqual(me.from("D notsuperset C or B subset A").default_order().tree);

    expect(
      me.from("A superseteq B or C notsubseteq D").default_order().tree,
    ).toEqual(

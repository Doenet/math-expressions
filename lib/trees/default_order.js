import { normalize_negative_numbers } from "../expression/normalization/index.js";
import { get_unit_value_of_tree } from "../expression/units.js";
...
function remove_duplicate_negatives(tree) {
  ...
  if (operator === "-" && typeof operands[0] === "number" && operands[0] < 0) {
    return -operands[0];
  }
...
function normalize_negatives_in_factors(tree) {
  ...
  for (var i = 0; i < operands.length; i++) {
    ...
    } else if (typeof operands[i] === "number" && operands[i] < 0) {
      sign *= -1;
...
function normalize_negatives(expr_or_tree) {
  ...
  tree = remove_duplicate_negatives(tree);
  tree = normalize_negative_numbers(tree);
...
function sort_key(tree, params = {}) {
  ...
  if (operator === "^" && operands.length === 2) {
    // sort key for power is based on the base, then the exponent

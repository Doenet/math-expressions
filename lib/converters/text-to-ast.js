...
function parse(input, context, pars ) {
    ...
    // parameter: splitSymbols
    // if true, split multicharacter symbols into a product of letters
    params.splitSymbols = get_parameter(
	"splitSymbols", pars, context_pars, defaults);
    
    // convert string "false" to false
    if(typeof params.splitSymbols === "string")
	params.splitSymbols = (params.splitSymbols.toLowerCase() !== "false");
    
    // parameter: unsplitSymbols
    // list of symbols that won't be split into a product of letters
    // even if splitSymbols==true
    params.unsplitSymbols = get_parameter(
	"unsplitSymbols", pars, context_pars, defaults);

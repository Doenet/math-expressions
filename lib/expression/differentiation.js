...
var variables_in_ast = require('./variables')._variables_in_ast;
var substitute_ast = require("./substitution")._substitute_ast;
var simplify_ast = require("./simplify")._simplify_ast;
...
function derivative_of_ast(tree,x,story) {
	...
	if (typeof input == "number") {
	    ...
	    story.push( 'Recall \\(\\frac{d}{du}' + astToLatex( example_ast ) + ' = ' +
			astToLatex( derivative_of_ast( example_ast, 'u', [] ) ) + '\\).' );
...
exports.derivative = function(expr, x) {
    var story = [];
    return expr.context.from(derivative_of_ast( expr.tree, x, story ));
};

exports.derivative_story = function(expr, x) {
    var story = [];
    derivative_of_ast( expr.tree, x, story );

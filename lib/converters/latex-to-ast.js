...
const allowedLatexSymbolsDefault = ['alpha', 'beta', 'gamma', 'Gamma', 'delta', 'Delta', 'epsilon', 'zeta', 'eta', 'theta', 'Theta', 'iota', 'kappa', 'lambda', 'Lambda', 'mu', 'nu', 'xi', 'Xi', 'pi', 'Pi', 'rho', 'sigma', 'Sigma', 'tau', 'Tau', 'upsilon', 'Upsilon', 'phi', 'Phi', 'chi', 'psi', 'Psi', 'omega', 'Omega', 'partial', 'angle', 'circ', '%', '$', 'emptyset'];
...
class latexToAst {
  ...
  statement_a({ inside_absolute_value = 0, parse_absolute_value = true } = {}) {
    ...
    while (['IMPLIES', 'IMPLIEDBY', 'IFF', 'LEFTARROW', 'RIGHTARROW', 'LEFTRIGHTARROW'].includes(this.token.token_type)) {
        ...
        if (unit_object) {
          if (unit_object.prefix && ind < n_ops - 1) {
            ...
            if (ind === 0) {
              ...
              return ["*", ...operands.slice(0, ind).map(this.convert_units_in_term.bind(this)), unit_tree];
            }
            ...
            if (ind === 1) {
              ...
              unit_tree = ["unit", ["*", ...operands.slice(0, ind).map(this.convert_units_in_term.bind(this))], unit]
            }
            if (ind === n_ops - 1) {
              return unit_tree;
            } else {

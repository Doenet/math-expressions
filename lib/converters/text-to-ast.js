...
import { get_all_units } from "../expression/units.js";
...
class textToAst {
  ...
  expression(params) {
    if (this.token.token_type === "NOT" || this.token.token_type === "!") {
      this.advance();
      return ["not", this.expression(params)];
    }
    ...
    while (
      ["+", "-", "PM", "UNION", "INTERSECT", "PERP", "PARALLEL"].includes(
        this.token.token_type,
      )
    ) {
      let operation = this.token.token_type.toLowerCase();
      ...
      if (this.token.token_type === "-") {
        ...
        negative = true;
        this.advance();
      } else if (this.token.token_type === "PM") {
        operation = "+";
        pm_sign = true;
        this.advance();
      } else {

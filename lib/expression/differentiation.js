...
var substitute = require("../trees/basic").substitute;
...
function derivative(expr_or_tree,x,story) {
	...
	story.push( 'By the chain rule, \\(' + ddx + astToLatex( tree ) + ' = ' + astToLatex(substitute( ["apply",operands[0] + "'","x"], { "x": input } )) + " \\cdot " + ddx + astToLatex(input)  + '\\).' );
	...
	if (typeof input == "number") {
	    ...
	    story.push( 'Recall \\(\\frac{d}{du}' + astToLatex( example_ast ) + ' = ' +
			astToLatex( derivative( example_ast, 'u', [] ) ) + '\\).' );

	    story.push( 'By the chain rule, \\(' + ddx + astToLatex( tree ) + ' = ' + astToLatex(substitute( derivatives[operator], { "x": input } )) + " \\cdot " + ddx + astToLatex(input)  + '\\).' );
...
exports.derivative = function(expr, x) {
    var story = [];
    return derivative( expr, x, story );
};

exports.derivative_story = function(expr, x) {
    var story = [];

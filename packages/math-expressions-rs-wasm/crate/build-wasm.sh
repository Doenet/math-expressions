#!/usr/bin/env bash
# Build the wasm-bindgen package (nodejs target) and run the JS smoke test
# (PORTING_PLAN.md §13). Output lands in ../pkg (sibling of this crate, inside
# the math-expressions-rs-wasm package; git-ignored).
#
...
if command -v wasm-opt >/dev/null 2>&1; then
  wasm-opt -Oz ../pkg/math_expressions_wasm_bg.wasm -o ../pkg/math_expressions_wasm_bg.wasm
else
  echo "note: wasm-opt not found; skipping post-link -Oz pass"
fi
echo '{"type":"commonjs"}' > ../pkg/package.json
cp wasm-smoke.cjs ../pkg/smoke.cjs
node ../pkg/smoke.cjs

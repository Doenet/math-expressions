...
import { evaluate_to_constant } from "./evaluation.js";
import {
...
import { simplify_powers_in_roots } from "./simplify-powers-in-roots.js";
import {
  apply_root,
  pull_number_outside_root,
} from "./simplify-roots-shared.js";
import { contains_pm } from "./pm.js";
...
function simplify(expr_or_tree, assumptions, max_digits) {
  ...
  tree = simplify_integers_in_roots(tree);
...
/** Recursively simplifies integer factors inside radicals by pulling out perfect powers. */
function simplify_integers_in_roots(tree) {
  if (!Array.isArray(tree)) {
    return tree;
  }
  ...
  if (operator === "*") {
    // since simplify_integers_in_roots can turn roots into multiplications,
    // we first apply simplify_integers_in_roots to all the operands,
    // and then merge any resulting products while combining any number factors into a single number factor.
    let numberFactor = 1;
    const otherFactors = [];
    for (const operand of operands) {
      if (typeof operand === "number") {
        numberFactor *= operand;
      } else {
        const newFactor = simplify_integers_in_roots(operand);
        if (Array.isArray(newFactor) && newFactor[0] === "*") {
          if (typeof newFactor[1] === "number") {
            numberFactor *= newFactor[1];
            otherFactors.push(...newFactor.slice(2));
          } else {
            otherFactors.push(...newFactor.slice(1));
          }
        } else {
          otherFactors.push(newFactor);
        }
      }
    }

    if (otherFactors.length === 0) {
      // the product consisted entirely of numbers,
      // so return their product rather than an empty multiplication
      return numberFactor;
    }
  ...
  if (
    ...
    if (rootOperator === "cbrt") {
      ...
      if (Array.isArray(operands[1]) && operands[1][0] === "tuple") {
        if (Number.isInteger(operands[1][2]) && operands[1][2] > 0) {
          ...
          // we have a nth root with a non-integer or non-positive root, so quit
          return [
            "apply",
            "nthroot",
            ...operands.slice(1).map(simplify_integers_in_roots),
          ];
        }
      } else {
        root = 2;
    ...
    rootOperand = simplify_integers_in_roots(rootOperand);
    if (Number.isInteger(rootOperand)) {
      let { n_outside, n_inside } = pull_number_outside_root(rootOperand, root);

      if (n_outside !== 1) {
        return ["*", n_outside, apply_root(n_inside, root)];
      }
    } else if (
      Array.isArray(rootOperand) &&
      rootOperand[0] === "*" &&
      typeof rootOperand[1] === "number"
    ) {
      let { n_outside, n_inside } = pull_number_outside_root(
        rootOperand[1],
        root,
      );
      if (n_outside !== 1) {
        return [
          "*",
          n_outside,
          apply_root(["*", n_inside, ...rootOperand.slice(2)], root),
        ];
      }
    }
    return apply_root(rootOperand, root);
  }

  return [operator, ...operands.map(simplify_integers_in_roots)];
}
...
function collect_like_terms_factors(expr_or_tree, assumptions, max_digits) {
  ...
  function isNegativeNumber(s) {
    ...
    if (Array.isArray(s) && s[0] === "-" && typeof s[1] === "number" && s > 0)
      return true;
    return false;
  }
  ...
  // Merging a base/term that carries an independent ± is unsound: combining
  // `n*x + m*x → (n+m)*x` or `x^n * x^m → x^(n+m)` would tie two independent
  // sign choices together (e.g. `±x · ±x` has value set {x², -x²} but `(±x)²`
  // has only {x²}). Used to guard both the multiplicative power-combining
  // rules and the additive like-term rules below.
  const noPmBase = (v) => !contains_pm(v);
  ...
  transformations.push([
    ...
    {
      variables: {
        x: (v) => is_nonzero(v, assumptions) && noPmBase(v),
        n: isNumber,
        m: isNumber,
      },
      evaluate_numbers: true,
  ...
  transformations.push([
    ...
    ["^", "x", "n"],
    {
      variables: {
        x: true,
        n: (v) => Number.isInteger(v) && Number.isInteger(v / 2),
      },
      evaluate_numbers: true,
      max_digits: max_digits,
      allow_extended_match: true,
  ...
  // n*x + m*x -> (n + m)*x  (guarded by noPmBase: `±3 + ±3` must not become
  // `2·±3`, just as `±x · ±x` must not become `(±x)²` above)
  transformations.push([
    ["+", ["*", "n", "x"], ["*", "m", "x"]],
    ["*", ["+", "n", "m"], "x"],
    {
  ...
  // pm-specific rewrites — safe because each `pm` denotes an independent
  // sign choice with the symmetry `−(±x) = ±x`.
  //
  // -(±x) → ±x  (negation absorbs into pm; value set is unchanged)
  transformations.push([
    ["-", ["pm", "x"]],
    ["pm", "x"],
    { variables: { x: true } },
  ]);
  // c · ±x → ±(c·x), when c contains no pm of its own. Scaling distributes
  // across a single ± because c is a single value, not a set.
  transformations.push([
    ["*", "c", ["pm", "x"]],
    ["pm", ["*", "c", "x"]],
    {
  ...
  transformations.push([
    ...
    {
      variables: {
        a: (v) => is_negative(v, assumptions),
        x: true,
      },
      evaluate_numbers: true,
      max_digits: max_digits,
      allow_implicit_identities: ["x"],
    },
  ]);
  ...
  transformations.push([
    ["apply", "nthroot", ["tuple", ["*", "a", "x"], "n"]],
    ["-", ["apply", "nthroot", ["tuple", ["-", ["*", "a", "x"]], "n"]]],
    {
      variables: {
        a: (v) => is_negative(v, assumptions),
        x: true,
        n: (v) => Number.isInteger(v) && !Number.isInteger(v / 2) && v > 1,
      },
      evaluate_numbers: true,
      max_digits: max_digits,
      allow_implicit_identities: ["x"],
  ...
  // pull symbolic perfect-power factors out of radicals
  // (done with a dedicated single pass rather than pattern transformations,
  // which only pull one factor at a time and can terminate prematurely)
  tree = simplify_powers_in_roots(tree, assumptions);
  ...
  // and remove multiple divisions again
  // x * (y / z) -> (x * y) / z
  transformations.push([
    ["*", "x", ["/", "y", "z"]],
    ["/", ["*", "x", "y"], "z"],

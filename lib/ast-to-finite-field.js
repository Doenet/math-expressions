...
function finite_field_evaluate_ast(tree, bindings, modulus, level) {
    ...
    if (operator == "sec") {
	return finite_field_evaluate_ast(['/', 1, ['cos', operands[0]]], bindings, modulus, level);
    }
	...
	//var x = finite_field_evaluate_ast(operands[0], bindings, q, level - 1);
...
function astToFiniteField(tree, modulus) {
    return function(bindings) { return finite_field_evaluate_ast( tree, bindings, modulus, 0 ); };
}

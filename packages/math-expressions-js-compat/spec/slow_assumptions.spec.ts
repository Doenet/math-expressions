...
import { is_integer } from "../lib/assumptions/element_of_sets";
import { is_real } from "../lib/assumptions/element_of_sets";
import { is_complex } from "../lib/assumptions/element_of_sets";
import { is_nonzero } from "../lib/assumptions/element_of_sets";
import { is_nonnegative } from "../lib/assumptions/element_of_sets";
import { is_nonpositive } from "../lib/assumptions/element_of_sets";
import { is_positive } from "../lib/assumptions/element_of_sets";
import { is_negative } from "../lib/assumptions/element_of_sets";
...
let ordered_trees_equal = (a, b) =>
  trees.equal(default_order(a), default_order(b));
...
describe("add and get assumptions", function () {
  it("single variable", function () {
    ...
    expect(
      ordered_trees_equal(me.get_assumptions("x"), me.from("x>0").tree),
    ).toBeTruthy();
    expect(
      ordered_trees_equal(
        me.assumptions.get_assumptions("x"),
        me.from("x>0").tree,
      ),
    ).toBeTruthy();
    expect(
    ...
    expect(
      ordered_trees_equal(
        ...
        simplify(me.from("x<=0 and x > -2").tree),
      ),
    ).toBeTruthy();
    expect(
      ordered_trees_equal(
  ...
  it("multiple variables", function () {
    ...
    expect(
      ordered_trees_equal(
        me.get_assumptions("x"),
        me.from("x>=0 and x < y+1").evaluate_numbers().tree,
      ),
    ).toBeTruthy();
    expect(
      ordered_trees_equal(
        me.get_assumptions("y"),
        me.from("y > x-1 and y >-1").evaluate_numbers().tree,
      ),
    ).toBeTruthy();
    ...
    expect(
      ordered_trees_equal(
        ...
        me.from("a < b and b < c").tree,
      ),
    ).toBeTruthy();

    expect(
      ordered_trees_equal(
        me.assumptions.get_assumptions("b"),
        me.from("a < b and b < c").tree,
      ),
    ).toBeTruthy();
  ...
  element_interval_tests.forEach(function (input) {
    it("element interval: " + input, function () {
      ...
      expect(
        ordered_trees_equal(me.get_assumptions("x"), me.from(input[1]).tree),
      ).toBeTruthy();
      me.clear_assumptions();
    });
  });
  ...
  interval_tests.forEach(function (input) {
    it("interval containment: " + input, function () {
      ...
      expect(
        ordered_trees_equal(
          me.get_assumptions([["a", "b"]]),
          me.from(input[1]).tree,
        ),
      ).toBeTruthy();
      me.clear_assumptions();
    });
  ...
  it("avoid redundancies", function () {
    ...
    expect(
      ordered_trees_equal(
        me.get_assumptions("x"),
        simplify(me.from("x<=0 and x>-1").tree),
      ),
    ).toBeTruthy();
  ...
  it("generic assumptions", function () {
    ...
    expect(
      ordered_trees_equal(
        me.get_assumptions("x"),
        me.from("x elementof R").tree,
      ),
    ).toBeTruthy();
    expect(
      ordered_trees_equal(
        me.get_assumptions("y"),
        me.from("y elementof R").tree,
      ),
    ).toBeTruthy();
    expect(
      ordered_trees_equal(
        me.get_assumptions("z"),
        me.from("z elementof R").tree,
      ),
    ).toBeTruthy();

    me.add_assumption(me.from("x != 3"), true);
    expect(
      ordered_trees_equal(me.get_assumptions("x"), me.from("x != 3").tree),
    ).toBeTruthy();
    expect(
      ordered_trees_equal(
        me.get_assumptions("y"),
    ...
    expect(
      ordered_trees_equal(me.get_assumptions("y"), me.from("y < z").tree),
    ).toBeTruthy();
    expect(
      ordered_trees_equal(me.get_assumptions("z"), me.from("y < z").tree),
    ).toBeTruthy();

    me.clear_assumptions();
    me.assumptions.add_generic_assumption(me.from("x < y"));
    expect(
      ordered_trees_equal(me.get_assumptions("x"), me.from("x < y").tree),
    ).toBeTruthy();
    expect(me.get_assumptions("y")).toEqual(undefined);
    expect(
      ordered_trees_equal(me.get_assumptions("z"), me.from("z < y").tree),
    ).toBeTruthy();
    ...
    expect(
      ordered_trees_equal(
        me.get_assumptions("x"),
        me.from("x != 3 and x elementof R").tree,
      ),
    ).toBeTruthy();

    expect(
      ordered_trees_equal(me.get_assumptions("y"), me.from("y != 4").tree),
    ).toBeTruthy();
    ...
    expect(
      ordered_trees_equal(me.get_assumptions("c"), me.from("c < d").tree),
    ).toBeTruthy();
  ...
  it("adding and removing assumptions", function () {
    ...
    expect(
      ordered_trees_equal(me.get_assumptions("y"), me.from("y>1").tree),
    ).toBeTruthy();
    ...
    expect(
      ordered_trees_equal(
        me.get_assumptions("z"),
        me.from("z elementof Z and z > 2").tree,
      ),
    ).toBeTruthy();

    me.remove_assumption(me.from("z elementof Z"));
    expect(
      ordered_trees_equal(me.get_assumptions("z"), me.from("z > 2").tree),
    ).toBeTruthy();
    ...
    expect(
      ordered_trees_equal(me.get_assumptions("z"), me.from("z > 5").tree),
    ).toBeTruthy();
    ...
    expect(
      ordered_trees_equal(
        me.get_assumptions("w"),
        me.from("w elementof Z").tree,
      ),
    ).toBeTruthy();

    me.add_assumption(me.from("w > 3"), true);
    expect(
      ordered_trees_equal(me.get_assumptions("w"), me.from("w >3").tree),
    ).toBeTruthy();
  ...
  it("removing assumptions with generic", function () {
    ...
    expect(
      ordered_trees_equal(
        me.get_assumptions("a"),
        me.from("a elementof Z").tree,
      ),
    ).toBeTruthy();
    expect(
      ordered_trees_equal(
        me.get_assumptions("b"),
        me.from("b elementof Z").tree,
      ),
    ).toBeTruthy();
    ...
    expect(
      ordered_trees_equal(
        me.get_assumptions("a"),
        me.from("a elementof Z and a<1-b and b elementof Z").tree,
      ),
    ).toBeTruthy();
    expect(
      ordered_trees_equal(
        me.get_assumptions("b"),
        me.from("b elementof Z and b < 1-a and a elementof Z").tree,
      ),
    ).toBeTruthy();
    ...
    expect(
      ordered_trees_equal(
        me.get_assumptions("a"),
        me.from("a elementof Z and a<1-b").tree,
      ),
    ).toBeTruthy();
    expect(
      ordered_trees_equal(
        me.get_assumptions("b"),
        me.from("b<1-a and a elementof Z").tree,
      ),
    ).toBeTruthy();
  ...
  it("removing generic assumptions", function () {
    ...
    expect(
      ordered_trees_equal(me.get_assumptions("q"), me.from("a<q and q<b").tree),
    ).toBeTruthy();
    ...
    expect(
      ordered_trees_equal(
        me.get_assumptions("q"),
        me.from("a<q and q<b and q !=0").tree,
      ),
    ).toBeTruthy();
    expect(
      ordered_trees_equal(me.get_assumptions("r"), me.from("a<r and r<b").tree),
    ).toBeTruthy();
    ...
    expect(
      ordered_trees_equal(me.get_assumptions("r"), me.from("a<r").tree),
    ).toBeTruthy();
    ...
    expect(me.get_assumptions("r", { known_variables: ["a", "b"] })).toEqual(
      undefined,
    );
...
describe("is positive/negative/zero/real/complex", function () {
  ...
  var assumptions = [
    ...
    ["x > 0", true, false, false, true, true, true, true, true],
    ["x >= 0", true, false, undefined, undefined, undefined, true, true],
    ["x < 0", false, true, true, false, true, true, true],
    ["x <= 0", undefined, undefined, true, false, undefined, true, true],
    ["x = 0", true, false, true, false, false, true, true],
  ];
  ...
  var assumptions_negative = [
    ...
    ["x > 0", false, true, true, false, true, true, true],
    ["x >= 0", undefined, undefined, true, false, undefined, true, true],
    ["x < 0", true, false, false, true, true, true, true],
    ["x <= 0", true, false, undefined, undefined, undefined, true, true],
    ["x = 0", true, false, true, false, false, true, true],
  ];
  ...
  var sum_tests = [
    ...
    ["x > 0 and y > 0", true, false, false, true, true, true, true],
    ["x > 0 and y >= 0", true, false, false, true, true, true, true],
    [
      "x > 0 and y < 0",
      undefined,
    ...
    ["x < 0 and y < 0", false, true, true, false, true, true, true],
    ["x < 0 and y <= 0", false, true, true, false, true, true, true],
    ["x < 0 and y = 0", false, true, true, false, true, true, true],
    ...
    ["x = 0 and y = 0", true, false, true, false, false, true, true],
  ];
  ...
  var triple_sum_tests = [
    ["z > 0", undefined, undefined, undefined, undefined, undefined, undefined],
    [
    ...
    ["x > 0 and y > 0 and z > 0", true, false, false, true, true, true],
    ["x > 0 and y >= 0 and z > 0", true, false, false, true, true, true],
    [
      "x > 0 and y < 0 and z > 0",
      undefined,
  ...
  var product_tests = [
    ...
    ["y = 0", true, false, true, false, false, true, true],
    ...
    ["x elementof R and y = 0", true, false, true, false, false, true, true],
    ...
    ["x elementof C and y = 0", true, false, true, false, false, true, true],
    ...
    ["x = 0", true, false, true, false, false, true, true],
    ["x = 0 and y elementof R", true, false, true, false, false, true, true],
    [
      "x = 0 and y elementof R and y != 0",
      true,
      false,
    ...
    ["x = 0 and y elementof C", true, false, true, false, false, true, true],
    [
      "x = 0 and y elementof C and y != 0",
      true,
      false,
  ...
  var power_tests = [
    ...
    ["x > 0 and y elementof R", true, false, false, true, true, true, true],
    [
      "x > 0 and y elementof R and y != 0",
      true,
      false,
...
describe("assumptions", function () {
  ...
  var assumptions_negated = [
    ...
    ["not(x != 0)", true, false, true, false, false, true, true],
    [
      "not(not(x != 0))",
      undefined,
      undefined,
    ...
    ["not(x > 0)", undefined, undefined, true, false, undefined, true, true],
    ["not(not(x > 0))", true, false, false, true, true, true, true, true],
    ["not(x >= 0)", false, true, true, false, true, true, true],
    [
      "not(not(x >= 0))",
      true,
      false,
    ...
    ["not(x < 0)", true, false, undefined, undefined, undefined, true, true],
    ["not(not(x < 0))", false, true, true, false, true, true, true],
    ["not(x <= 0)", true, false, false, true, true, true, true, true],
    [
      "not(not(x <= 0))",
      undefined,
      undefined,
    ...
    ["not(not(x = 0))", true, false, true, false, false, true, true],
  ];
  ...
  var function_tests = [
    ...
    ["sqrt(x)", "x >0", true, false, false, true, true, true, true],
    [
      "exp(x)",
      "x elementof C",
      undefined,
    ...
    ["exp(x)", "x elementof R", true, false, false, true, true, true, true],
    ["exp(x)", "x >=0", true, false, false, true, true, true, true],
    ["exp(x)", "x >0", true, false, false, true, true, true, true],
    [
      "abs(x)",
      "x elementof C",
    ...
    ["abs(x)", "x >0", true, false, false, true, true, true, true],
    [
      "abs(x)",
      "x != 0 and x elementof C",
      true,
      false,
      false,
      true,
...
describe("derived assumptions", function () {
  it("combined equality", function () {
    ...
    expect(
      ordered_trees_equal(
        me.get_assumptions("x"),
        me.fromText("x=a and x=b and x=c").tree,
      ),
    ).toBeTruthy();
    expect(
      ordered_trees_equal(
        me.get_assumptions("a"),
        me.fromText("a=x and a=b and a=c").tree,
      ),
    ).toBeTruthy();
    expect(
      ordered_trees_equal(
        me.get_assumptions("b"),
        me.fromText("b=a and b=x and b=c").tree,
      ),
    ).toBeTruthy();
    expect(
      ordered_trees_equal(
        me.get_assumptions("c"),
        me.fromText("c=a and c=b and c=x").tree,
      ),
    ).toBeTruthy();
  ...
  it("combined inequality", function () {
    ...
    expect(
      ordered_trees_equal(
        me.get_assumptions("b"),
        me.fromText("b>x and b>a and b<c").tree,
      ),
    ).toBeTruthy();
    expect(
      ordered_trees_equal(
        me.get_assumptions("c"),
        me.fromText("c>x and c>a and c>b").tree,
      ),
    ).toBeTruthy();
    ...
    expect(
      ordered_trees_equal(me.get_assumptions("x"), me.fromText("x<a").tree),
    ).toBeTruthy();
    expect(
      ordered_trees_equal(
        me.get_assumptions("a"),
        me.fromText("a>x and a>=b").tree,
      ),
    ).toBeTruthy();
    expect(
      ordered_trees_equal(
        me.get_assumptions("b"),
        me.fromText("b<=a and b<=c").tree,
      ),
    ).toBeTruthy();
    expect(
      ordered_trees_equal(me.get_assumptions("c"), me.fromText("c >=b").tree),
    ).toBeTruthy();
  ...
  it("combine containment", function () {
    ...
    expect(
      ordered_trees_equal(
        me.get_assumptions("x"),
        me.fromText(
          "x notelementof A and x notelementof B and x notelementof C",
        ).tree,
      ),
    ).toBeTruthy();
    ...
    expect(
      ordered_trees_equal(
        me.get_assumptions("A"),
        me.fromText(
          "A subset B and A subset C and x notelementof A and E notsubset A",
        ).tree,
      ),
    ).toBeTruthy();
    ...
    expect(
      ordered_trees_equal(
        me.get_assumptions("A"),
        me.fromText(
          "A superset B and A superset C and x elementof A and E notsuperset A",
        ).tree,
      ),
    ).toBeTruthy();
  ...
  it("don't retrieve additional", function () {
    ...
    expect(
      ordered_trees_equal(me.get_assumptions("a"), me.fromText("a>b").tree),
    ).toBeTruthy();
    expect(
      ordered_trees_equal(
        me.get_assumptions("b"),
...
describe("assumptions on expressions", function () {
  it("linear inequality", function () {
    ...
    expect(
      ordered_trees_equal(me.get_assumptions("q"), me.fromText("q > x").tree),
    ).toBeTruthy();
    expect(
      ordered_trees_equal(
    ...
    expect(
      ordered_trees_equal(
        me.get_assumptions(me.from("q-a").tree),
        me.fromText("q-a > 0 and q-a > q-x and q-a > x-a and a elementof Z")
          .tree,
      ),
    ).toBeTruthy();
    ...
    expect(
      ordered_trees_equal(
        ...
        me
          .fromText("3a+4b > 0 and 3a+4b > 2c+6d and -3d < c and c/-3 < d")

...
export const equals = function ({
  ...
  if (contains_pm(expr.tree) || contains_pm(other.tree)) {
    return pm_equals_numerical({
      ...
      other,
      randomBindings,
      expr_context: expr_context || expr.context,
      other_context: other_context || other.context,
      relative_tolerance,
      absolute_tolerance,
      tolerance_for_zero,
      rng,
...
// multisets of values produced by the sign-expansions of each side coincide
// (within tolerance). This function expands all `pm` operators into their
// 2^n sign assignments, then samples bindings and compares value multisets.
function pm_equals_numerical({
  ...
  try {
    expr_variant_trees = expand_pm_signs(expr.tree);
    other_variant_trees = expand_pm_signs(other.tree);
  } catch (e) {
    // too many pm operators to enumerate; cannot decide
    return false;
  }

  const expr_variants = expr_variant_trees.map((t) => expr_context.fromAst(t));
  const other_variants = other_variant_trees.map((t) =>
    other_context.fromAst(t),
  );

  // Use the first variant of each side to determine variables/functions —
  // sign-expansion does not change which symbols appear.
  let variables, integer_variables, functions;
  let prepared_expr_variants, prepared_other_variants;
  try {
    const prepared = prepare_exprs_numerical_eval(
      expr_variants[0],
      other_variants[0],
      expr_context,
      other_context,
    ...
    integer_variables = prepared.integer_variables;
    functions = prepared.functions;
  } catch (e) {
    return false;
  }

  // Renormalize every variant the same way the first ones were normalized,
  // so that variable/function sets are consistent across variants.
  prepared_expr_variants = expr_variants.map((e) =>
    normalize_for_equality_checking(e, e, expr_context, expr_context).expr,
  );
  prepared_other_variants = other_variants.map((e) =>
    normalize_for_equality_checking(e, e, other_context, other_context).expr,
  );

  let expr_fs, other_fs;
  try {
    expr_fs = prepared_expr_variants.map((e) => e.f());
    other_fs = prepared_other_variants.map((e) => e.f());
  } catch (e) {
    return false;
  }
  ...
  for (let i = 0; i < 10 * number_tries; i++) {
    ...
    for (let k = 0; k < functions.length; k++) {
      ...
      const c3 = (rng() - 0.5) * 6;
      const c4 = (rng() - 0.5) * 24;
      bindings[functions[k]] = function (x) {
        return math.add(
          math.multiply(
            math.add(math.multiply(math.add(math.multiply(c1, x), c2), x), c3),
            x,
...
function value_multisets_match(
  ...
  if (values_a.length !== values_b.length) {
    // Different cardinality is still allowed if values repeat — fall back
    // to the bidirectional "every value in A matches some value in B" check.
    return (
      values_a.every((va) =>
        values_b.some((vb) =>
          values_close(
...
function values_close(
  ...
  let tol = min_mag * relative_tolerance;
  tol = Math.min(tol, 0.1 * min_mag);
  if (tol === 0 && (a === 0 || b === 0)) {
    tol += tolerance_for_zero;
  } else {

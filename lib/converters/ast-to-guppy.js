...
class astToGuppy {
  constructor() {
    this.operators = {
      ...
      "/": function (operands) {
        return astToGuppy.dfrac(operands[0], operands[1]);
      },
      "^": function (operands) {
        return astToGuppy.power(operands[0], operands[1]);
      },
      sin: function (operands) {
        return astToGuppy.trig("sin", operands[0]);
      },
      cos: function (operands) {
        return astToGuppy.trig("cos", operands[0]);
      },
      tan: function (operands) {
        return astToGuppy.trig("tan", operands[0]);
      },
      arcsin: function (operands) {
        return astToGuppy.trig("arcsin", operands[0]);
      },
      arccos: function (operands) {
        return astToGuppy.trig("arccos", operands[0]);
      },
      arctan: function (operands) {
        return astToGuppy.trig("arctan", operands[0]);
      },
      arccsc: function (operands) {
        return astToGuppy.trig("arccsc", operands[0]);
      },
      arcsec: function (operands) {
        return astToGuppy.trig("arcsec", operands[0]);
      },
      arccot: function (operands) {
        return astToGuppy.trig("arccot", operands[0]);
      },
      csc: function (operands) {
        return astToGuppy.trig("csc", operands[0]);
      },
      sec: function (operands) {
        return astToGuppy.trig("sec", operands[0]);
      },
      cot: function (operands) {
        return astToGuppy.trig("cot", operands[0]);
      },
      log: function (operands) {
        return astToGuppy.trig("log", operands[0]);
      },
      exp: function (operands) {
        return astToGuppy.trig("exp", operands[0]);
      },
      ln: function (operands) {
        return astToGuppy.trig("ln", operands[0]);
      },
      sqrt: function (operands) {
        return astToGuppy.sqrt(operands[0]);
      },
      abs: function (operands) {
        return astToGuppy.abs(operands[0]);
      },
  ...
  */
  ...
  factor(tree) {
    ...
    if (operator === "abs") {
      ...
      if (
        operator === "this.factorial" &&
        (operands[0].toString().length === 1 ||
          operands[0].toString().match(/^[0-9]*$/))
      )
  ...
  */
  ...
  expression(tree) {
    ...
    if (operator === "+" || operator === "-") {
      return this.operators[operator](
        operands.map(
          function (v, i) {
            return this.factorWithParenthesesIfNegated(v);
          }.bind(this),
        ),
      );
    }
  ...
  convert(tree) {
    return ("<m><e></e>" + this.expression(tree) + "<e></e></m>").replace(
      /<\/e><e>/g,
      "",
    );
  }

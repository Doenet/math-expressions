...
import { normalize_display_negative_fractions } from "./normalize-display-negative-fractions.js";
import { expandScientificNotation } from "./expand-scientific-notation.js";
...
class astToText {
  constructor({
    ...
    showBlanks = true,
    // When true, render all multiplication with explicit `*` separators.
    explicitMultiplicationSymbols = false,
  } = {}) {
    ...
    this.explicitMultiplicationSymbols = explicitMultiplicationSymbols;
  }

  convert(tree) {
    return this.statement(normalize_display_negative_fractions(tree));
  }
  ...
  term(tree) {
    ...
    if (operator === "*") {
      let termFactors = operands.map(
        function (v, i) {
          let result;
          if (i > 0) {
            result = this.factorWithParenthesesIfNegatedOrExplicitPlus(v);
            if (!this.explicitMultiplicationSymbols && result.match(/^[0-9]/)) {
              return "* " + result;
      ...
      if (this.explicitMultiplicationSymbols) {
        return termFactors.join("*");
      }

      return this.operators[operator](termFactors);
    }
  ...
  simple_factor_or_function_or_parens(tree) {
    ...
    // or tree is a non-negative number, and either not in scientific notation
    //   or avoidScientificNotation is enabled
    // or tree is a string
    ...
    if (
      ...
      if (
        tree >= 0 &&
        (this.avoidScientificNotation || !tree.toString().includes("e"))
      ) {
        return true;
      } else {
        return false;
  ...
  factor(tree) {
    ...
    if (typeof tree === "number") {
      if (tree === Infinity) {
        ...
        if (eIndex === -1 || this.avoidScientificNotation) {
          if (eIndex !== -1 && this.avoidScientificNotation) {
            numberString = expandScientificNotation(numberString);
          }
  ...
  termWithPlusIfNotNegated(tree) {
    ...
    if (result.match(/^± /) || result.match(/^plusminus /)) return result;

...
function expand(expr_or_tree, no_division) {
  ...
  // `pm`-containing operands of an odd power likewise inflates the
  // value set, e.g. `(±k+m)^3 → ±k^3 + 3k^2 m + ±(3k m^2) + m^3` has
  // sign-combinations that the original cannot produce. So guard the
  // outer factor / power base against `pm`.

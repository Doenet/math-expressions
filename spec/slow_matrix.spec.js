...
describe("matrixtest", function () {
  it("creation", function () {
    ...
    expect(matrix.tree).toEqual([
      "matrix",
      ["tuple", 2, 2],
      ["tuple", ["tuple", "x1", "x2"], ["tuple", "x3", "x4"]],
    ]);
  });
});
...
describe("vector, tuple addition", function () {
  ...
  vectors.forEach(function (example) {
    it(example.toString(), function () {
      ...
      expect(me.vector_add(first_vector, second_vector).tree).toEqual(
        sum_vector.tree,
      );
      expect(
        me.vector_add(first_vector, second_vector.tuples_to_vectors()).tree,
      ).toEqual(sum_vector.tree);
      expect(
        me.vector_add(first_vector.tuples_to_vectors(), second_vector).tree,
      ).toEqual(sum_vector.tree);
      expect(
        me.vector_add(
          first_vector.tuples_to_vectors(),
          second_vector.tuples_to_vectors(),
        ).tree,
      ).toEqual(sum_vector.tree);
    });
  });
...
describe("altvector addition", function () {
  var vectors = [
    ...
    [
      ...
      "\\langle 5,2,1,2 \\rangle",
      "\\langle 6,4,4,6 \\rangle",
    ],
  ];
...
describe("scalar times vector", function () {
  ...
  vectors.forEach(function (example) {
    it(example.toString(), function () {
      ...
      expect(me.scalar_mul(scalar, vector.tuples_to_vectors()).tree).toEqual(
        prod_vector.tree,
      );
    });
  });
...
describe("vector subtraction", function () {
  ...
  vectors.forEach(function (example) {
    it(example.toString(), function () {
      ...
      expect(me.vector_sub(first_vector, second_vector).tree).toEqual(
        sum_vector.tree,
      );
      expect(
        me.vector_sub(
...
describe("cross product", function () {
  ...
  vectors.forEach(function (example) {
    it(example.toString(), function () {
      ...
      expect(me.cross_prod(first_vector, second_vector).tree).toEqual(
        cross_vector.tree,
      );
      expect(
        me.cross_prod(

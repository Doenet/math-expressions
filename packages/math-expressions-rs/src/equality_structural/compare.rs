...
use super::forms::{
    has_integration_constant, is_completed_square, is_expanded, is_factored_completely,
    like_terms_remain,
};
use super::fractions::{
    contains_decimal, is_decimal_number, is_improper_fraction, is_mixed_number,
    is_reduced_fraction, is_single_fraction,
};
...
pub fn check_structural_comparison(
    e: &Expr,
    check: &StructuralComparison,
    ...
    match check {
        ...
        StructuralComparison::MixedNumber => StructuralComparisonResult::of(
            is_mixed_number(e),
            "not written as a mixed number a + b/c",
        ),
        ...
        StructuralComparison::Expanded => {
            StructuralComparisonResult::of(is_expanded(e), "not fully expanded")
        }
        StructuralComparison::FactoredCompletely => {
            StructuralComparisonResult::of(is_factored_completely(e), "not completely factored")
        }
        StructuralComparison::SingleFraction => StructuralComparisonResult::of(
            is_single_fraction(e),
            "not written as a single fraction",
        ),
        StructuralComparison::NoNegativeExponents => StructuralComparisonResult::of(
            !has_negative_exponent(e),
        ...
        StructuralComparison::CompletedSquare => StructuralComparisonResult::of(
            is_completed_square(e),
            "not in completed-square shape a(x-h)^2 + k",
        ),
...
pub fn structural_equality(
    ...
    match comparison {
        ...
        _ => {
            check_structural_comparison(student, comparison).ok && crate::equals(student, key, opts)
        }
    }
}

...
import * as flatten from "../lib/trees/flatten";
...
describe("tree basics", function () {
  ...
  it("(1+2+3) unflattens right to (1+(2+3))", function () {
    expect(
      trees.equal(flatten.unflattenRight(["+", 1, 2, 3]), [
        "+",
        1,
        ["+", 2, 3],
      ]),
  ...
  it("(1+2+3) unflattens left to ((1+2)+3)", function () {
    expect(
      trees.equal(flatten.unflattenLeft(["+", 1, 2, 3]), ["+", ["+", 1, 2], 3]),
    ).toBeTruthy();
  });

  it("(1+(2+3)) flattens to (1+2+3)", function () {
    expect(
      trees.equal(flatten.flatten(["+", 1, ["+", 2, 3]]), ["+", 1, 2, 3]),
    ).toBeTruthy();
  });
  ...
  it("x+y becomes 1+2 when x:=1 and y:=2", function () {
    expect(
      trees.equal(trees.substitute(["+", "x", "y"], { x: 1, y: 2 }), [
        "+",
        1,
        2,
      ]),
  ...
  it("x+y becomes y^2+x^2 when x := y^2 and y := x^2", function () {
    expect(
      trees.equal(
        trees.substitute(TREE("x+y"), { x: TREE("y^2"), y: TREE("x^2") }),
        TREE("y^2 + x^2"),
      ),
    ).toBeTruthy();
  });
  ...
  it("substituting cos(x+y)/sin(x*y) with x:=1 and y:=2", function () {
    expect(
      trees.equal(
        trees.substitute(TREE("cos(x+y)/sin(x*y)"), { x: 1, y: 2 }),
        TREE("cos(1+2)/sin(1*2)"),
      ),
    ).toBeTruthy();
  });
  ...
  it("substituting x*y/(x^y) with x := (y+1) and y := (x+2)", function () {
    expect(
      trees.equal(
        trees.substitute(TREE("x*y/(x^y)"), { x: TREE("y+1"), y: TREE("x+2") }),
        TREE("(y+1)*(x+2)/((y+1)^(x+2))"),
      ),
    ).toBeTruthy();
  });
...
describe("tree matching", function () {
  ...
  it("specify variables", function () {
    expect(trees.match(TREE("x+y"), TREE("a+b"))).toBeTruthy();
    expect(
      trees.match(TREE("x+y"), TREE("a+b"), {
        variables: { a: true, b: true },
      }),
    ).toBeTruthy();
    expect(
      trees.match(TREE("x+y"), TREE("a+b"), { variables: { a: true } }),
    ).toBeFalsy();
    expect(
      trees.match(TREE("x+y"), TREE("a+b"), { variables: { b: true } }),
    ).toBeFalsy();
    expect(me.fromText("x+y").match(me.fromText("a+b"))).toBeTruthy();
    expect(
      me
        .fromText("x+y")
        .match(me.fromText("a+b"), { variables: { a: true, b: true } }),
    ).toBeTruthy();
    expect(
      me.fromText("x+y").match(me.fromText("a+b"), { variables: { a: true } }),
    ).toBeFalsy();
    expect(
      me.fromText("x+y").match(me.fromText("a+b"), { variables: { b: true } }),
    ).toBeFalsy();
  });
  ...
  it("matching with function conditions", function () {
    ...
    expect(
      trees.match(TREE("2x+y"), pattern, {
        variables: { a: true, b: isString },
      }),
    ).toBeTruthy();
    expect(trees.match(TREE("x+2y"), pattern)).toBeTruthy();
    expect(
      trees.match(TREE("x+2y"), pattern, {
        variables: { a: true, b: isString },
      }),
    ).toBeFalsy();

    expect(
      trees.match(TREE("x+2"), pattern, {
        variables: { a: isString, b: isNumber },
      }),
    ).toBeTruthy();
    expect(
      trees.match(TREE("x+2"), pattern, {
        variables: { b: isString, a: isNumber },
      }),
    ).toBeFalsy();
  });
  ...
  it("matching with regular expression conditions", function () {
    ...
    expect(
      trees.match(TREE("2x+3y"), pattern, { variables: { a: true, b: /y/ } }),
    ).toBeTruthy();
    expect(
      trees.match(TREE("2x+3y"), pattern, { variables: { a: true, b: /^y$/ } }),
    ).toBeFalsy();
    expect(
      trees.match(TREE("2x+3y"), pattern, { variables: { a: /.*/, b: /y/ } }),
    ).toBeTruthy();

    expect(
      trees.match(TREE("2x+3y"), pattern, {
        variables: { a: true, b: /^[a-zA-Z]$/ },
      }),
    ).toBeFalsy();
    expect(
      trees.match(TREE("2x+3y"), TREE("a+3b"), {
        variables: { a: true, b: /^[a-zA-Z]$/ },
      }),
    ).toBeTruthy();
    ...
    expect(
      trees.match(TREE("x+2"), pattern, {
        variables: { a: /^[a-zA-Z]$/, b: /^\d+$/ },
      }),
    ).toBeTruthy();
    expect(
      trees.match(TREE("x+2"), pattern, {
        variables: { b: /^[a-zA-Z]$/, a: /^\d+$/ },
      }),
    ).toBeFalsy();
  });
  ...
  it("invalid matching conditions fail gracefully", function () {
    expect(
      trees.match(TREE("2x+y"), TREE("a+b"), {
        variables: { a: false, b: "h" },
      }),
    ).toBeFalsy();
  });
  ...
  it("match with permutation", function () {
    ...
    expect(
      trees.match(TREE("e^(0.3s^2+3s+7)"), pattern, {
        variables: {
          a: isNumber,
          b: isNumber,
          c: isNumber,
    ...
    expect(
      me.fromText("e^(0.3s^2+3s+7)").match(pattern, {
        variables: {
          a: isNumber,
          b: isNumber,
          c: isNumber,
    ...
    expect(
      me.fromText("e^(7+3s+s^2*0.3)").match(pattern, {
        variables: {
          a: isNumber,
          b: isNumber,
          c: isNumber,
  ...
  it("match with implicit identity", function () {
    ...
    expect(
      trees.match(TREE("s^2-3s+7"), pattern, {
        variables: {
          a: isNumber,
          b: isNumber,
          c: isNumber,
  ...
  it("consistency with with implicit identity", function () {
    ...
    expect(
      trees.match(TREE("s^2+1s+7"), pattern, {
        variables: {
          a: isNumber,
          b: isNumber,
          c: isNumber,
    ...
    expect(
      trees.match(TREE("s^2+s+7"), pattern, {
        variables: {
          a: isNumber,
          b: isNumber,
          c: isNumber,
  ...
  it("match chunks", function () {
    ...
    expect(match).toBeTruthy();
    expect(
      trees.equal(match["a"], TREE("xyz")) &&
        trees.equal(match["b"], TREE("5a-2")),
    ).toBeTruthy();

    match = trees.match(TREE("xyz+5a-2"), pattern, {
      variables: { a: true, b: isNumber },
    });

    expect(match).toBeTruthy();
    expect(
      trees.equal(match["a"], TREE("xyz+5a")) &&
        trees.equal(match["b"], TREE("-2")),
    ).toBeTruthy();
    ...
    expect(match).toBeTruthy();
    expect(
      trees.equal(default_order(match["b"]), default_order(TREE("xyz+5a"))) &&
        trees.equal(match["a"], TREE("-2")),
    ).toBeTruthy();

    pattern = TREE("a+b+c");

    match = trees.match(TREE("xyz+5a-2+2q-3/u"), pattern, {
    ...
    match = trees.match(TREE("xyz+7+5a-2+2q-3/u"), pattern, {
      variables: { a: isNumber, b: true, c: isNumber },
      allow_permutations: true,
    });
  ...
  it("extended match", function () {
    ...
    var match = trees.match(TREE("x+z/2+y+1+y"), TREE("b+a+b"), {
      variables: { a: isNumber, b: true },
    });

    expect(match).toEqual(false);

    match = trees.match(TREE("x+z/2+y+1+y"), TREE("b+a+b"), {
      variables: { a: isNumber, b: true },
      allow_permutations: true,
    });
    ...
    match = trees.match(TREE("x+z/2+1+y+y"), TREE("b+a+b"), {
      variables: { a: isNumber, b: true },
      allow_extended_match: true,
    });
    ...
    match = trees.match(TREE("x+y+z/2+1+y"), TREE("b+a+b"), {
      variables: { a: isNumber, b: true },
      allow_extended_match: true,
    });
    ...
    match = trees.match(TREE("x+y+1+y+z/2"), TREE("b+a+b"), {
      variables: { a: isNumber, b: true },
      allow_extended_match: true,
    });
...
describe("tree transformations", function () {
  it("replacing x*y in x*y+z with y/x results in y/x+z", function () {
    ...
    expect(
      trees.equal(
        trees.replaceSubtree(root, subtree, replacement),
        TREE("y/x + z"),
      ),
    ).toBeTruthy();
  });
  ...
  it("applying commutativity to a+b+c finds some permutations", function () {
    ...
    var results = trees.applyTransformationEachSubtree(
      tree,
      pattern,
      replacement,
    );
    ...
    expect(
      (trees.equal(results[0], left) && trees.equal(results[1], right)) ||
        (trees.equal(results[1], left) && trees.equal(results[0], right)),
    ).toBeTruthy();
  });

  it("applying commutativity and associativity finds some equalities", function () {
    var commutativity = trees.patternTransformer(
      ["+", "a", "b"],
      ["+", "b", "a"],
    );
    var associativity = trees.patternTransformer(
      ["+", "a", ["+", "b", "c"]],
      ["+", ["+", "a", "b"], "c"],
    );
    ...
    expect(
      trees.equalAfterTransformations(left, right, [
        commutativity,
        associativity,
      ]),
    ).toBeTruthy();
  ...
  it("applying commutativity and associativity finds some non-equalities", function () {
    ...
    expect(
      trees.equalAfterTransformations(
        left,
        right,
  ...
  it("Apply distributive transformations to expand polynomial", function () {
    ...
    expect(
      trees.equal(default_order(expanded), default_order(transformed)),
    ).toBeTruthy();
  });

  it("trig transformation", function () {
    var transformationTrig = [TREE("cos(b)^2+sin(b)^2"), 1];

    var original = TREE("cos(2y)^2+1+sin(2y)^2+y/2");

    var result = trees.applyAllTransformations(original, [transformationTrig]);

    expect(trees.equal(result, original)).toBeTruthy();

    transformationTrig = [
      TREE("cos(b)^2+sin(b)^2"),
      1,
      { allow_extended_match: true, allow_permutations: true },
    ];
    result = trees.applyAllTransformations(original, [transformationTrig]);
  ...
  it("combine like terms", function () {
    ...
    result = trees.applyAllTransformations(TREE("3x+4y-x*2+qx-3x-y+x"), [
      transformation,
    ]);

    expect(
      trees.equal(default_order(result), default_order(TREE("-x+3y+qx"))),
    ).toBeTruthy();
  });
  ...
  it("make sure replaces with 0", function () {
    ...
    var result = trees.applyAllTransformations(TREE("z^0z^5"), [
      transformation,
    ]);

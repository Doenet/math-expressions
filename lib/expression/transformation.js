...
function expand(expr_or_tree, no_division) {
  ...
  tree = perform_vector_scalar_multiplications(tree);
...
function perform_matrix_multiplications(tree, include_vectors = true, include_tuples = true) {
  ...
  tree = trans.transform(tree, function (subtree) {
    ...
    if (matchResults) {
      ...
      if (postFactors.length > 0 || postFactors.length > 0) {
...
function perform_vector_scalar_multiplications(tree, include_tuples = true) {

  let vector_scalar_mult_transform = function (ast, operator) {

    let pattern = ['*', [operator, 'vectorData'], "b"];
    ast = trans.transform(ast, function (subtree) {
      ...
      if (matchResults) {
        ...
        while (factors.length > 0) {
          let operator2 = factors[0][0];
          if (["tuple", "list", "vector", "interval", "set", "array", "matrix"].includes(operator2)) {
            break;
          } else {
            if (factors[0] !== 1) {
              vectorData = vectorData.map(x=> ["*", x, factors[0]])
            }
            factors.splice(0, 1);
          }
        }
